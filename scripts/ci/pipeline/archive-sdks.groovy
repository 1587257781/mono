    ...
    ws ("workspace/${jobName}/${monoBranch}/${configuration}") {
        timestamps {
            ...
            try {
                ...
                def packageUrl = "https://xamjenkinsartifact.azureedge.net/mono-sdks/"
                currentBuild.description = "<hr/><h2>DOWNLOAD: <a href=\"${packageUrl}/${packageFileName}\">${packageFileName}</a></h2><hr/>"

                utils.reportGitHubStatus (isPr ? env.ghprbActualCommit : commitHash, "Archive-${product}-${configuration}-${platform}", "${packageUrl}/${packageFileName}", 'SUCCESS', packageFileName)
            }
            catch (Exception e) {
